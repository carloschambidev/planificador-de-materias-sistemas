...
import { MATERIAS, getMateriaById } from "../../core/data/materias";
import { getElectivaById } from "../../core/data/electivas";
import {
  cuentaComoAprobada,
  ...
  const materias = useMemo<MateriaCompleta[]>(() => {
    return MATERIAS.map((def) => {
      ...
      if (def.esElectiva && dinamico.electivaAsignadaId) {
        ...
        if (electivaDef) {
          reqReg = electivaDef.requisitos?.paraCursar?.regularizadas ?? electivaDef.regularizadasRequeridas ?? [];
          reqApr = electivaDef.requisitos?.paraCursar?.aprobadas ?? electivaDef.aprobadasRequeridas ?? [];
        }
      }
